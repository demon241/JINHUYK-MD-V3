...
if (!isAdmins) return reply(ADMIN)	
  if (args[1] == 'second') {
                ...
                setTimeout(() => {
                    ...
                    const open = `*OPEN TIME* THE GROUP WAS OPENED BY JINHUYK-MD-V3 TO APPROVED ADMIN\n NOW MEMBERS CAN SEND MESSAGES 🔓`
...
if (!isAdmins) return reply(ADMIN)	
                ...
                setTimeout(() => {
                    ...
                    const close = `*CLOSE TIME* GROUP CLOSED BY JINHUYK-MD-V3 AT APPROVED ADMIN\nNOW ONLY ADMIN CAN SEND MESSAGES 🔐`
...
try{
        ...
        if (!isGroup) return reply(`Cette commande est uniquement destinée aux groupes.`);
        if (!isAdmins) return reply(`Cette commande est uniquement destinée à l'administrateur du groupe.`);
        
        // Fetch all group admins
        const admins = groupAdmins;
        if (admins.length === 0) {
            return reply('Il ny a pas dadministrateurs dans ce groupe.');
        }
        // Create a message with all admin tags
        let adminTagMessage = '*ÉTIQUETAGE DE TOUS LES ADMINISTRATEURS DU GROUPE 🔳:*\n\n';
...
if (!isBotAdmins) return reply(mg.needbotadmins);     
           ...
           const mass = await conn.sendMessage(m.chat, { text: '*GROUP CHAT MUTED BY JINHUYK-MD-V3* 🔒' }, { quoted: mek });
...
if (!isBotAdmins) return reply(mg.needbotadmins);     
           ...
           const mass = await conn.sendMessage(m.chat, { text: '*GROUP CHAT UNMUTED BY JINHUYK-MD-V3* 🔒' }, { quoted: mek });
            return await conn.sendMessage(m.chat, { react: { text: '🔒', key: mass.key } });
} catch(e) {
console.log(e);
reply('*PLEASE GIVE ME A ADDMIN ROLE❗👻*')    
